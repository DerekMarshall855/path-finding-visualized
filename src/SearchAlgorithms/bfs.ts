export const bfs = (grid: any, startCell: any, endCell: any) => {
    const visitedCellsInOrder = [];
    let nextCellsStack = [startCell];
    while (nextCellsStack.length) {
        const currentCell = nextCellsStack.shift();
        if (currentCell === endCell) return visitedCellsInOrder;
